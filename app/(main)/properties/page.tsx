...
export default async function PropertiesPage({
  ...
  const [{ data: properties }, { data: totalCount }, { data: amenities }] =
    ...
    // Map amenities to our app `Amenity` shape (ensure non-nullable fields)
    const mappedAmenities = (amenities || []).map((a: any) => ({
      _id: a._id,
      value: a.value ?? "",
      label: a.label ?? "",
      icon: a.icon ?? null,
    ...
    // Map properties to our app `Property` shape (fill defaults for missing fields)
    const mappedProperties = (properties || []).map((p: any) => ({
      ...
      propertyType: (p.propertyType as any) ?? ("house" as const),
      status: (p.status as any) ?? ("active" as const),
      bedrooms: typeof p.bedrooms === "number" ? p.bedrooms : 0,
      bathrooms: typeof p.bathrooms === "number" ? p.bathrooms : 0,
      squareFeet: typeof p.squareFeet === "number" ? p.squareFeet : 0,
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="lg:w-80 flex-shrink-0">
            <div className="lg:sticky lg:top-24">
              ...
              >
                <FilterSidebar amenities={mappedAmenities} />
              </Suspense>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0">
            ...
            <Tabs defaultValue="list" className="w-full">
              ...
              <TabsContent value="map" className="mt-0">
                  ...
                  <DynamicMapView properties={mappedProperties || []} />
                </div>
              </TabsContent>
            </Tabs>
          </div>
